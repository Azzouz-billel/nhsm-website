...
The **owner** (superuser) also gets `/manage/owner/`: visitors online right now (a 5-minute
presence window tracked by `config.middleware.PresenceMiddleware` — signed-in members by name,
guests counted) plus site-wide totals and the latest signups / uploads / requests / ratings.
A **"Needs your attention"** panel at the top lists everything waiting on a decision (pending
resources, reviews, comments, open requests) with direct links.
`config.middleware.AnalyticsMiddleware` additionally keeps **per-day traffic** (unique visitors,
page views, average time on site — cached live, rolled into `DailyStats` at day change), and
library cards open through `/resources/<id>/open/`, so the owner page also shows **per-day
traffic and the most opened resources (overall and by semester)**. Both are approximate by
design — no third-party tracker.
...
2. **Auth + user profiles + in-app moderation** — ✅ done. Register / login / logout
   ...
   localStorage); resource upload + approver review queue at `/moderation/`. Password recovery:
   every user gets a **recovery code** (shown after sign-up and on the profile); reset at
   `/accounts/forgot/` with username + code, and logged-in users can change their password from
   the profile. Lost codes are regenerated by an admin from `/manage/users/`.
3. **Pomodoro focus timer + tracking** — ✅ done. `/timer/` with configurable focus/break
   (default 25/5, long break every 4), module picker **or a hand-written activity label**
   (e.g. "Revision" — saved without a module), progress ring, and a **focus mode** button that
   covers the whole screen with just the timer (Esc to leave). Completed focus blocks
